...
# Define label list
label_list = ['Positive', 'Negative', 'Not Important']
...
if uploaded_file is not None:
    ...
    st.image(complete_image, caption="Uploaded Image", use_column_width=True)
    
    # Let the user select patch resolution
    col1, col2 = st.columns(2)
    patch_height = col1.number_input("Select patch height", min_value=16, max_value=complete_image.size[1], value=512, step=16)
    patch_width = col2.number_input("Select patch width", min_value=16, max_value=complete_image.size[0], value=512, step=16)
    
    if st.button("Generate Patches"):
    ...
    if st.session_state['patches'] is not None:
        ...
        st.session_state['patch_index'] = st.selectbox(
            "Select a patch to display", 
            options=list(range(len(st.session_state['patches']))), 
            format_func=lambda x: f"Patch {x + 1}", 
